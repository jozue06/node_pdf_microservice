...
class LayoutBuilder {
        ...
        function addPageBreaksIfNecessary(linearNodeList, pages) {
            ...
            return linearNodeList.some(function (node, index, followingNodeList) {
                if (node.pageBreak !== 'before' && !node.pageBreakCalculated) {
                    ...
                    let followingNodesOnPage = followingNodeList.slice(index + 1).filter(function (node0) {
                        return node0.nodeInfo.pageNumbers.indexOf(pageNumber) > -1;
                    });
                    let nodesOnNextPage = followingNodeList.slice(index + 1).filter(function (node0) {
                        return node0.nodeInfo.pageNumbers.indexOf(pageNumber + 1) > -1;
                    });
                    let previousNodesOnPage = followingNodeList.slice(0, index).filter(function (node0) {
                        return node0.nodeInfo.pageNumbers.indexOf(pageNumber) > -1;
                    });
                    if (pageBreakBeforeFct(node.nodeInfo, followingNodesOnPage.map(function (node) {
    ...
    addBackground(background) {
        let backgroundGetter = isFunction(background) ? background : function () {
            return background;
        };
    ...
    addHeadersAndFooters(header, footer) {
        let headerSizeFct = function (pageSize, pageMargins) {
            return {
                x: 0,
                y: 0,
        ...
        let footerSizeFct = function (pageSize, pageMargins) {
            return {
                x: 0,
    ...
    processList(orderedList, node) {
        let self = this, items = orderedList ? node.ol : node.ul, gapSize = node._gapSize;
...
function decorateNode(node) {
	...
	if (isArray(node.canvas)) {
		node.canvas.forEach(function (vector) {
			let x = vector.x, y = vector.y, x1 = vector.x1, y1 = vector.y1, x2 = vector.x2, y2 = vector.y2;
